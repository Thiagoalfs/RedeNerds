...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($servername === '' || $title === '' || $descricao === '' || $modpackurl === '' || $ip === '') {
        ...
        try {
            ...
            $servidor['ip']         = $ip;
            $servidor['themecolor'] = $themecolor;
            $servidor['enabled']    = $enabled ? 1 : 0;
...
<head>
    ...
    <title>Editar Servidor - Painel Administrativo</title>
    ...
    <div class="container-fluid px-3 px-md-4 my-3 my-md-4">
        <div class="card shadow-sm">
            <div class="card-header bg-white p-3 p-md-4">
                <h4 class="mb-0 h5 h-md-4">✏️ Editar Servidor #<?php echo (int)$servidor['id']; ?></h4>
            </div>
            <div class="card-body p-3 p-md-4">
                ...
                <form method="POST" action="servidor_editar.php?id=<?php echo (int)$servidor['id']; ?>" autocomplete="off" enctype="multipart/form-data">
                    ...
                    <div class="card bg-light p-3 mb-3">
                        ...
                        <small class="text-muted d-block mb-2">Preencha um dos campos abaixo para trocar o ícone atual, ou deixe todos em branco para mantê-lo.</small>
                        <div class="row g-3">
                        ...
                        <div class="mt-3">
                            <div class="preview-icone" id="icone-preview-box">
                                <?php if (tipoDoIcone($servidor['icon']) === 'img'): ?>
                                    <img src="<?php echo htmlspecialchars($servidor['icon'], ENT_QUOTES, 'UTF-8'); ?>" alt="ícone atual" onerror="this.parentNode.innerHTML='<i class=\'fa-solid fa-image text-muted\'></i>'">
                                <?php else: ?>
                                    <i class="<?php echo htmlspecialchars($servidor['icon'] ?: 'fa-solid fa-image text-muted', ENT_QUOTES, 'UTF-8'); ?>"></i>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
