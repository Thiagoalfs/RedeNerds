...
if ($id <= 0) {
    ...
    header("Location: /admin/chaves/?erro=" . urlencode("ID de Chave inválido."));
    exit;
}

if (isset($pdo) && $pdo instanceof PDO) {
    try {
        $stmt = $pdo->prepare("SELECT ativo, nome FROM chaves WHERE id = :id LIMIT 1");
        $stmt->execute([':id' => $id]);
        $chave = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($chave) {
            $novoStatus = $chave['ativo'] ? 0 : 1;
            $upd = $pdo->prepare("UPDATE chaves SET ativo = :novo WHERE id = :id");
            ...
            $msg = $novoStatus ? "Chave '{$chave['nome']}' ativada com sucesso!" : "Chave '{$chave['nome']}' desativada com sucesso!";
            header("Location: /admin/chaves/?msg=" . urlencode($msg));
            exit;
        } else {
            if ($isAjax) {
        ...
        error_log("Erro ao alternar status da chave: " . $e->getMessage());
        if ($isAjax) {
